...
class AuthenticateManager {
  ...
  invalidateUserInfoTimestamp = () => {
    localStorage.setItem(
      LOCAL_STORAGE_USER_INFO_TIMESTAMP,
  ...
  getUserDataFromLocalStorage = () => {
    ...
    if (userInfo) {
      let userInfoTimestamp = localStorage.getItem(
        LOCAL_STORAGE_USER_INFO_TIMESTAMP
      );
      if (userInfoTimestamp) {
        // 유효기간 체크
        if (
          new Date().getTime() - Number(userInfoTimestamp) >
          LOCAL_STORAGE_USER_INFO_VALID_TIME
        ) {
          userInfo = null;
          this.removeUserDataFromLocalStorage();
        } else {
